...
namespace Planets.Services
{
    ...
    {
        public Planet[] GetAllPlanets(string solarSystem)
        {
            if (solarSystem == "Solar")
            {
                // Hardcoded, this would be a stored procedure call to the DB
                return new Planet[]
                {
                    new Planet
                    {
        ...
        public Planet GetPlanet(int planetId, string system)
        {
            return GetAllPlanets(system).FirstOrDefault(x => x.Id == planetId);
        }
    }
}